...
type PickerModel struct {
	...
	// e.g. a first-run nudge. Set by the caller before Run().
	Banner string

	// WantSettings is set when the user pressed `s` on the picker. The parent
	// CLI reads this after the program exits and relaunches with the settings
	// model.
	WantSettings bool
}

func NewPicker(cfg *pipeline.Config) (*PickerModel, error) {
...
// initialParallel reflects the live cfg into the config-screen slider.
// Falls back to 1 when cfg hasn't been parallelized yet so the slider
// isn't sitting above the "serial" floor on a default run.
func initialParallel(cfg *pipeline.Config) int {
...
func qualityIndex(q pipeline.Quality) int {
	switch q {
	case pipeline.QualityLow:
		return 0
	case pipeline.QualityHigh:
		return 2
	}
	return 1 // medium is the default
}

func compressionIndex(c pipeline.Compression) int {
	switch c {
	case pipeline.CompressionFast:
		return 0
	case pipeline.CompressionBest:
		return 2
	}
	return 1 // balanced is the default
}

// discoverVideos scans sourceDir for known video extensions. When recursive,
// the walk skips hidden dirs and ivideo-hls's own workspaces. Items show the
// path relative to sourceDir so nested files can be distinguished.
func discoverVideos(dir string, recursive bool) ([]videoItem, error) {
	paths, err := pipeline.ScanVideos(dir, recursive)
	if err != nil {
		return nil, err
	}
...
// visibleItems returns indices (into m.items) that match the current filter.
// When no filter is active, every index is returned.
...
func (m *PickerModel) commitConfig() {
	qualities := []pipeline.Quality{pipeline.QualityLow, pipeline.QualityMedium, pipeline.QualityHigh}
	compressions := []pipeline.Compression{pipeline.CompressionFast, pipeline.CompressionBalanced, pipeline.CompressionBest}

	m.Cfg.Videos = m.selectedPaths()
	m.Cfg.MaxParallel = m.parallel
	m.Cfg.ParallelMode = m.parallel > 1
	m.Cfg.Quality = qualities[m.quality]
	m.Cfg.Compression = compressions[m.compression]
	m.Cfg.PreCompress = m.preCompress
	m.Cfg.KeepSource = m.keepSource
	m.Confirmed = true
}
...
func (m *PickerModel) renderSourceHeader() string {
	...
	parts = append(parts, styleDim.Render("  ·  "+mode+"  ·  "+fmt.Sprintf("%d types", len(pipeline.VideoExtensions))))
...
func (m *PickerModel) renderEmptyDirPanel() string {
	...
	body += "\n" + styleError.Render("No .mp4 files found in this folder.") + "\n\n" +
		...
		styleDim.Render("Or relaunch with --source /path/to/videos (or -i file.mp4).")
	panel := stylePanel.Width(m.panelWidth()).Render(body)
...
func configHint(field int) string {
	switch field {
	case cfgFieldParallel:
		return "Concurrent encodes (CPU pool). Push pool auto-sized to 2× — network is the cheap stage."
	case cfgFieldQuality:
